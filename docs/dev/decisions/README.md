...
### 2026-06-02
- `[forte]` Fix — [`lim`/`sup`/… : opérande n-aire `▒` pour ne pas happer le 1er token](2026-06-02-Fix-lim-naryand.md) — Word rendait `\lim_{x\to0}\frac{1}{x+1}` en `\frac{\lim 1}{x+1}` (lim happe `1`, le `/` en fait le numérateur). Cause : `lim` opérateur n-aire en lettres, opérande non délimité. Fix conversion : insérer le n-aryand `▒` (U+2592) après l'indice de `lim`/`sup`/`inf`/`max`/`min` → `lim_(x→0)▒1/(x+1)`. Bug de RENDU → validation finale en Word. (Core legacy : 89 échecs pré-existants `Patterns.Templates.*`, sans rapport.)
