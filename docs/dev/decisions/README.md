...
- `[molle]` Fix — [Score : écho de symétrie entre frères à signatures prolongées (« 1/2x + 1/2x2 »)](2026-06-10-Fix-sibling-echo-symmetry.md) — le top était l'hybride `\frac{1}{2x}+\frac{1}{2}x^2` (optima locaux divergents, la cohérence existante ne couple que les signatures identiques) ; nouveau terme `SiblingEcho` : frères dont la signature de l'un prolonge l'autre → même forme de tête bonus, divergentes malus → top symétrique `\frac{1}{2x}+\frac{1}{2x^2}`, 5 fixtures, zéro collatéral sur 325.
- `[molle]` Fix — [Hygiène de suppression des anchors CC (caret piégé, orphelines)](2026-06-10-Fix-anchor-cc-deletion-hygiene.md) — trois défenses locales : Backspace/Suppr sélectionnent anchor+OMath comme une unité (suppression atomique), balayeur d'orphelines ±4 positions au caret, éjection immédiate d'un caret entré dans la CC cachée (fini la frappe invisible).
