...
- [x] **P5R** — Refacto convention args espace : retrait des 6 openers, nouveau `ArgListPatternBase` abstrait, `ForallBelongsTemplate` hérite, discrimination var/domain = dernier arg = ensemble identifié. ADR [`Refactor-forall-belongs-arglist-convention`](../decisions/2026-05-21-Refactor-forall-belongs-arglist-convention.md). Supersedes P5 partiellement. **Test PILOTE `V x [0,1]U[3,4]` → ∀x ∈ [0,1]∪[3,4] vert bout-en-bout.** Base abstraite réutilisée par futurs `LimTemplate`/`SumTemplate`/`IntegralTemplate` en P9+.
