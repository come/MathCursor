...
- [x] **P5** — `ForallBelongsTemplate` : heads V/E/∀/∃, slot var CSV, slot domain optionnel (6 openers pondérés), composition via PatternRegistry, P4.5 intégré (EnsembleTemplate head `[` délègue à interval-union), mutation composite, structures data-ready prêtes pour YAML. ADR [`Feat-forall-belongs-pattern`](../decisions/2026-05-21-Feat-forall-belongs-pattern.md). +47 tests verts. **Test PILOTE de l'ADR cadrage `V x app a [0,1]U[3,4]` → ∀x ∈ [0,1]∪[3,4] vert bout-en-bout.**
