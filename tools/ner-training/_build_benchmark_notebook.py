"""
Génère train_mathcursor_benchmark.ipynb à partir de cellules définies ici.

Usage one-shot — à relancer si on veut modifier le notebook de benchmark.
"""

import json
from pathlib import Path

OUT = Path(__file__).resolve().parent / "train_mathcursor_benchmark.ipynb"
...
cells.append(md(
    "# MathCursor — Benchmark 3 modèles NER (corpus v3)\n"
    "\n"
    "Teste **XLM-RoBERTa-base**, **DistilBERT multilingue** et **MiniLM multilingue**\n"
    "sur le même corpus enrichi v3 (v2 + fixtures projet + `²` + anti-FP).\n"
    "\n"
    "Critère d'adoption d'un modèle plus léger :\n"
    "- F1 ≥ 0.98 sur `test.jsonl`\n"
    "- 100% sur fixtures projet (`extension_v3_fixtures.jsonl`)\n"
    ...
    "   `train.jsonl`, `val.jsonl`, `test.jsonl`, `extension_v3_fixtures.jsonl`,\n"
    "   `extension_v3_superscript2.jsonl`, `extension_v3_false_positives.jsonl`\n"
    "3. Runtime → GPU T4 → Tout exécuter\n"
    "\n"
    "Durée : ~10-15 min (3 modèles × 4 epochs sur ~6700 lignes)."
))

cells.append(md("## 1. Drive + vérification fichiers"))
...
cells.append(md("## 3. Paramètres — 3 candidats"))

cells.append(code(
    "MODELS = [\n"
    "    {'name': 'xlm-roberta-base',                       'short': 'xlmr',      'lr': 3e-5},\n"
    "    {'name': 'distilbert-base-multilingual-cased',     'short': 'distilmult','lr': 5e-5},\n"
    "    {'name': 'microsoft/Multilingual-MiniLM-L12-H384', 'short': 'minilm',    'lr': 5e-5},\n"
    "]\n"
    "\n"
    "MAX_LENGTH = 128\n"
    "BATCH_SIZE = 16\n"
...
cells.append(md("## 4. Chargement corpus v3 (v2 + extensions)\n\n"
...
cells.append(code(
    ...
    "    }\n"
    "\n"
    "train_v2 = load_jsonl(os.path.join(WORKDIR, 'train.jsonl'))\n"
    "val      = load_jsonl(os.path.join(WORKDIR, 'val.jsonl'))\n"
    "test     = load_jsonl(os.path.join(WORKDIR, 'test.jsonl'))\n"
    "ext_fix  = load_jsonl(os.path.join(WORKDIR, 'extension_v3_fixtures.jsonl'))\n"
    "ext_sq   = load_jsonl(os.path.join(WORKDIR, 'extension_v3_superscript2.jsonl'))\n"
    "ext_fp   = load_jsonl(os.path.join(WORKDIR, 'extension_v3_false_positives.jsonl'))\n"
    "\n"
    "train_v3 = train_v2 + ext_fix + ext_sq + ext_fp\n"
    "\n"
    "datasets = DatasetDict({\n"
    "    'train':      Dataset.from_dict(to_dict(train_v3)),\n"
    "    'validation': Dataset.from_dict(to_dict(val)),\n"
    "    'test':       Dataset.from_dict(to_dict(test)),\n"
    "    'gold':       Dataset.from_dict(to_dict(ext_fix)),  # fixtures seules pour éval dédiée\n"
    "})\n"
    "\n"
    "print(f'Train v3: {len(train_v3)} lignes')\n"
    "print(f'  dont train v2:            {len(train_v2)}')\n"
    "print(f'  dont extension fixtures:  {len(ext_fix)}')\n"
    "print(f'  dont extension ²:         {len(ext_sq)}')\n"
    "print(f'  dont extension anti-FP:   {len(ext_fp)}')\n"
    "print(f'Val:   {len(val)}')\n"
    "print(f'Test:  {len(test)}')\n"
    "print(f'Gold (fixtures eval only): {len(ext_fix)}')"
))

cells.append(md("## 5. Tokenisation + alignement BIO (fonction réutilisable par modèle)"))
...
cells.append(md("## 7. Fonction train_and_eval\n\n"
...
cells.append(md("## 8. Boucle sur les 3 modèles"))
...
cells.append(md("## 9. Tableau final & recommandation"))

cells.append(code(
    "print(f'\\n{\"=\"*90}')\n"
    "print(f'  BENCHMARK FINAL — corpus v3')\n"
    "print(f'{\"=\"*90}\\n')\n"
    "\n"
    ...
    "\n"
    "print()\n"
    "print('Critère d\\'adoption : F1 test ≥ 0.98 ET F1 gold = 1.0')\n"
    ...
    "    print(f'\\n✅ Recommandation : {best[\"short\"]} ({best[\"name\"]})')\n"
    ...
    "else:\n"
    "    print('\\n⚠️  Aucun candidat ne passe le critère. Garder la baseline XLM-R avec corpus v3.')\n"
    "\n"
    "with open(os.path.join(RESULTS_DIR, 'benchmark.json'), 'w') as f:\n"
    "    json.dump(results, f, indent=2)"
))

cells.append(md("## 10. Archive du modèle recommandé\n\n"
...
cells.append(code(
    ...
    "\n"
    "if winner:\n"
    "    archive_base = os.path.join(WORKDIR, f'mathcursor-ner-int8-{winner[\"short\"]}')\n"
...
print(f"Écrit : {OUT.name} ({OUT.stat().st_size/1024:.1f} Ko, {len(cells)} cellules)")
