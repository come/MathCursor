...
internal static class Lexer
{
    ...
    public static List<Token> Lex(string src, EngineCulture culture, Func<int, bool>? choices = null, List<int>? runsOut = null)
    {
        ...
        while (i < src.Length)
        {
            ...
            // raccourcis grecs « @ » (ADR 2026-06-15) : @ + lettre(s) → atome grec.
            // Lettre SEULE = raccourci (alias case-insensible) ; la CASSE tapée
            // choisit min/MAJ : @d→δ, @D→Δ, @t→[θ,τ], @T→Θ, @P→[Π,Φ,Ψ]. Lettre
            // sans capitale grecque distincte (@A, @E…) ou nom complet (@theta,
            // @Delta) → Word (qui gère la casse). @ sans lettre derrière reste
            ...
            if (c == '@' && IsAlpha(Ch(i + 1)))
            {
                ...
                if (gr.Length == 1)
                {
                    bool up = IsUpper(gr[0]);
                    string low = "@" + char.ToLowerInvariant(gr[0]);
                    string key = culture.Canon(low);
                    if (key != low)
                    {
                        var ve = Vocabulary.Vocab[key];
                        if (ve.Alts != null)
                        {
                            var alts = up ? ve.AltsUpper : ve.Alts;
                            if (alts != null) { Push(new Token { Kind = "atom", Syms = alts }); continue; }
                        }
                        else if (!up || ve.Upper != ve.Lower)
                        {
                            Push(new Token { Kind = "atom", Sym = up ? ve.Upper : ve.Lower });
                            continue;
                        }
                    }
                }
                Word(gr, gsp);   // nom complet (@theta, @Delta) ou repli lettre latine (@A)
                continue;
            }
